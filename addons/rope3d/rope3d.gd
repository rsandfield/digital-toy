...
var middle_point: Vector3 :
	get:
		...
		return (
			...
			(end_point.global_transform.origin - start_point.global_transform.origin) /
			2.0
		)
...
func _prepare() -> bool:
	...
	if resolution > 2:
		...
		var before_turning_length := (
			(before_turning_count + 0.5) *
			_joint_length + before_turning_count * segment_length
		)
...
#		_turning_point_draw = turning_point
		...
		for i in len(segments):
			...
			if i <= _turning_index:
				...
				var k: float = (
					(i + 1) * _joint_length + i * segment_length + 0.5 * segment_length
				) / before_turning_length
				...
				var k: float = (
					(i - before_turning_count + 1) * _joint_length + segment_length
				) / after_turning_length
...
func _add_joints():
	...
	for i in len(segments):
		if segments.size() >= i + 2:
			...
			var sphere_middle_point = (
				segment.global_transform.origin +
				(next_segment.global_transform.origin - segment.global_transform.origin) *
				0.5
			)
			joint.global_transform.origin = sphere_middle_point


func _generate_joint_between(
	body_a: PhysicsBody3D, body_b: PhysicsBody3D, priority: int = 1
) -> PinJoint3D:
...
func _update_mesh():
	rope_mesh.path.curve.set_point_position(0, to_local(start_point.global_position))
	rope_mesh.path.curve.set_point_position(
		rope_mesh.path.curve.point_count - 1,
		to_local(end_point.global_position)
	)
