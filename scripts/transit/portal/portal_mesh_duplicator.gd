...
func find_by_class(node: Node, name_of_class : String, exit_after_one = false, result_array = []):
    if node.is_class(name_of_class):
        result_array.push_back(node)
        if exit_after_one:
            return result_array
    for child in node.get_children():
        find_by_class(child, name_of_class, exit_after_one, result_array)
    return result_array

func get_child_by_name(node: Node, child_name):
...
func synchronize_all():
    ...
    var original_rel_to_in_portal = (
        in_portal.global_transform.affine_inverse() *
